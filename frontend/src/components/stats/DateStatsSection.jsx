...
const EMPTY_FILTERS = { langs: [], speaker: '', audioIds: [], limit: LIMIT_DEFAULT };
...
function formatDateLabel(date, fallbackLabel) {
  if (!date) return fallbackLabel || 'Без даты';
...
function DateStatsSection() {
  ...
  const loadData = async (activeFilters) => {
    setLoading(true);
    try {
      const result = await statsApi.fetchDateWordStats(activeFilters);
      setData(result);
    } catch (error) {
      console.error('Ошибка загрузки статистики по датам:', error);
      toast.error('Не удалось загрузить статистику по датам');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <StatsSection
      ...
      description="Сколько слов было произнесено в каждый день записи. Листайте график в сторону, если дат много."
    >
        ...
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '13px', color: colors.textMuted, alignItems: 'center' }}>
          {data && !loading && (
            <>
              ...
              <span>Дат: <b style={{ color: colors.text }}>{data.total_dates}</b></span>
            </>
          )}
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <div ref={filterWrapRef} style={{ position: 'relative', flexShrink: 0 }}>
            ...
            {filtersOpen && (
              <div
                role="dialog"
                aria-label="Фильтры статистики по датам"
                style={{
                  position: 'absolute',
                  top: 'calc(100% + 8px)',
                  right: 0,
              ...
              >
                ...
                <div style={{ marginBottom: '18px' }}>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px', color: colors.textMuted }}>Количество дат на графике</label>
                  <input
                    type="number"
                    min={LIMIT_MIN}
                    max={LIMIT_MAX}
                  ...
                  <div style={{ marginTop: '6px', fontSize: '12px', color: colors.textFaint }}>
                    От {LIMIT_MIN} до {LIMIT_MAX}. Показываются последние даты с данными.
                  </div>
                </div>
...
export default DateStatsSection;

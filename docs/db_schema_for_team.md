...
Для фильтров дата, слово, говорящий, язык схема достаточна: recorded_at, words.text, words.language, speakers и words.speaker_id уже есть, дополнительные таблицы не требуются. Осталось только заполнять recorded_at (прокинуть при загрузке) и speaker_id, для которого нужен отдельный шаг диаризации; до него поле пустое. Остальное — слова, метрики, частоты, сегменты — пишется в db_index.save_transcription уже сейчас.
